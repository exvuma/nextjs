    ...
    <Layout>
      ...
      <BlogPost title='Publishing Typescript to NPM' date='Feburary 29th, 2019'>
        ...
        <p>
          ...
          with <code>npm pack</code>.
        </p>
        <p>Inside package to publish:</p>
        <pre>
          <code>
            {' '}
            ~/package $ npm pack ... npm notice === Tarball Details === npm
            notice name: package npm notice version: 0.0.0 npm notice filename:
            package-0.0.0.tgz ....
          </code>
        </pre>

        <p>
          With the tar just generated at{' '}
          <code>~/package/package-0.0.0.tar</code>, Inside test project with
          dependency:
        </p>
        ...
        <ul>
          ...
          <li>
            In you package.json, make sure <code>prepack</code> is a script set
            to <code>npm run build</code>
          </li>
        </ul>
...
`}
        ...
        <p>
          ...
          made the mistake just using npm link. <code>npm link</code> is great
          for developing packages where you want to test the client fast, but it
          doesn't replicate the build steps of publishing on npm. This lead us
          to publishing to npm with just the <code>.ts</code> files and without
          the corerect <code>.js</code> and <code>.d.js</code> files needed for
        ...
        <p>
          ...
          <code>npm pack</code> will create a tar ball that performs all the
          steps NPM does on publish (<code>prepack</code>,{' '}
          <code>post publish</code>, ..) . For more information on what scripts
          are ran when with npm see{' '}
        ...
        <p>
          Now we have a local version ready of the tar file that will actually
          be published on NPM without having to publish.{' '}
        </p>

        <h2 id='testtheinstall'>Test the Install</h2>

        <p>
          To install the local tarball run <code>npm install</code> with the
          path of the <code>.tgz</code> parent directory:
        </p>
        ...
        <p>
          Testing the install now gives us a really funky and useless error
          code:
        </p>
        ...
        <pre>
          <code>
            resolve 'package' in
            '/Users/victoriabernard/test/project/workers-site' Parsed request is
            a module using description file:
            ...
            doesn't exist or is not a directory /Users/node_modules doesn't
            exist or is not a directory /node_modules doesn't exist or is not a
            ...
            file: /Users/victoriabernard/test/project/package.json (relative
            path: ./node_modules) Field 'browser' doesn't contain a valid alias
        ...
        <p>
          ...
          went on and on. I was very confused and assumed this had something to
          do with the fact Workers uses a framework based on Service Workers and
          not node or vanilla browser JS.
        </p>

        <p>
          After some intense googling I realized that this probably had nothing
          to do with browsers, node, or service worker but that something could
          be going on with confused imports.
        </p>
        ...
        <pre>
          <code>
            {' '}
            ~/package $ npm pack npm notice npm notice 📦 package@0.0.0 npm
            notice === Tarball Contents === npm notice 90B .prettierrc npm
            notice 9.7kB LICENSE_APACHE npm notice 1.1kB LICENSE_MIT npm notice
            940B package.json npm notice 290B tsconfig.json npm notice 4.4kB
            ...
            notice 489B .github/workflows/test.yml npm notice === Tarball
            Details === npm notice name: package npm notice version: 0.0.0 npm
            notice filename: package-0.0.0.tgz npm notice package size: 11.2 kB
            npm notice unpacked size: 36.7 kB npm notice shasum:
        ...
        <p>
          ...
          <code>.js</code> files. I really need to get better at ready the
          output of things. <code>prepack</code> needed to be set to npm run
          build.
        </p>
    ...
    "prepack": "npm run build"
        ...
        <pre>
          <code>
            ...
            === Tarball Details === npm notice name:
            ...
            size: 146.1 kB npm notice unpacked size: 174.0 kB npm notice shasum:
        ...
        <pre>
          <code>
            ...
            dist/test/getAssetFromKV.js npm notice 10.8kB dist/index.js npm
            notice 4.1kB dist/test/mapRequestToAsset.js npm notice 4.8kB
            dist/mocks.js npm notice 4.5kB dist/test/serveSinglePageApp.js npm
            notice 2.6kB dist/types.js npm notice 1.1kB package.json npm notice
            290B tsconfig.json npm notice 4.4kB CHANGELOG.md npm notice 6.9kB
            README.md npm notice 185.0kB cloudflare-kv-asset-handler-0.0.8.tgz
            npm notice 11B dist/test/getAssetFromKV.d.ts npm notice 7.6kB
            src/test/getAssetFromKV.ts npm notice 1.6kB dist/index.d.ts npm
            notice 7.0kB src/index.ts npm notice 11B
            dist/test/mapRequestToAsset.d.ts npm notice 927B
            src/test/mapRequestToAsset.ts npm notice 465B dist/mocks.d.ts npm
            notice 1.7kB src/mocks.ts npm notice 11B
        ...
        <p>
          I test in back in the project folder, and see the import's are
          resolving correctly.{' '}
        </p>
      </BlogPost>
    </Layout>
